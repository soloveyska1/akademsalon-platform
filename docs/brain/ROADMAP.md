...
### OUT-008 — Телефон получает собственный продукт, а не сжатый десктоп

- Почему важно: мобильный слой описан как вычитание из десктопа — 638 правил
  `max-width` ≤920 px по всем CSS, а `assets/css/mobile.css` (3354 строки, 5% от
  64356 строк CSS) прячет контент 45 правилами `display:none` и ужимает его 119
  переопределениями `font-size`. Установки на домашний экран, офлайна и иконок
  нет вовсе.
- Результат: ключевые пути (главная → досье → конфигуратор → заявка, кабинет,
  каталог) спроектированы от мобильного вверх, а не урезаны из десктопа; сайт
  устанавливается на домашний экран, отвечает без сети и остаётся пригодным для
  последующей нативной упаковки без второго переписывания.
- Связи: `OUT-003`, `OUT-005`, `OUT-006`, `OUT-007`, `SUR-001`, `UXD-0008`.
- Метрика: на 390×844 P0/P1=0; интерактивные цели ≥44×44 px; поля ввода ≥16 px
  без зума iOS; горизонтальных протечек body нет; открытая клавиатура не прячет
  активное поле и primary; повторный запуск без сети даёт осмысленный ответ, а
  не ошибку браузера.
- Proof: измеренный аудит на iOS Simulator и в профилях устройств, затем
  failing-first контракты на тач-цели, поля, протечки и офлайн, полный
  regression, production smoke и проверенный rollback.
- Gate: сначала аудит с числами и согласованный владельцем план; переписывание
  экранов не начинается до утверждения плана. Нативная упаковка (Capacitor,
  App Store, Google Play) в этот outcome не входит.

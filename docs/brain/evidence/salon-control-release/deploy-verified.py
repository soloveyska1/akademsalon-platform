...
base=Path('/var/www/academic_saloon');stage=Path('/tmp/salon-control-release200');archive=stage/'salon-public-release200-delta.tar.gz';manifest=json.loads((stage/'salon-public-release200.manifest.json').read_text())
assert hashlib.sha256(archive.read_bytes()).hexdigest()=='972ac2f4cd14af20c767a3e78c0e06067c9ef6ea69618cc210e00b38e733736b'
assert hashlib.sha256((stage/'salon-public-release200.manifest.json').read_bytes()).hexdigest()=='fe56df7478f256dc4ebcc66dbd3f1df04fea05b0083a0e7e58d0863a3b3dc623'
assert manifest['source_commit']=='4dfb5dc3270eb1ddb60e2f373252c17150358817'
old=base/'releases/release199-c8f7afaa';release=base/'releases/release200-4dfb5dc3'
assert (base/'current').resolve()==old and (base/'dist').resolve()==old
assert not release.exists()
unpack=Path(tempfile.mkdtemp(prefix='.release200-',dir=base/'releases'))

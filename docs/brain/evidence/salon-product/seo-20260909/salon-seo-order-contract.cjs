...
return {out:JSON.parse(JSON.stringify(ctx.OUT)),writes,M:ctx.SalonCommerce}}
...
fs.writeFileSync('/tmp/salon-seo-order-contract.json',JSON.stringify({passed:results.length,tests:results,scope:'Exact source initialization + unmodified commerce functions; no DOM rendering or submission'},null,2));console.log(JSON.stringify({passed:results.length}));

...
base=Path('/var/www/academic_saloon');stage=Path('/tmp/salon-seo-release194');archive=stage/'salon-public-release194-delta.tar.gz';manifest=json.loads((stage/'salon-public-release194.manifest.json').read_text())
assert hashlib.sha256(archive.read_bytes()).hexdigest()=='483abb6a652cb2e7d211ec1eaa2b98b63a5f7a63da94ce62aa5a49dfe42a5fc3'
assert hashlib.sha256((stage/'salon-public-release194.manifest.json').read_bytes()).hexdigest()=='193d17c2d2a7402285c509cbcd3b52e9647d3dcbeb37d0d46d24b627d508ba8f'
assert manifest['source_commit']=='43d385012eab92d2805e3429a02cac2c4e51cd98'
old=base/'releases/release193-b2590b25';release=base/'releases/release194-43d38501'
assert (base/'current').resolve()==old and (base/'dist').resolve()==old
assert not release.exists()
unpack=Path(tempfile.mkdtemp(prefix='.release194-',dir=base/'releases'))

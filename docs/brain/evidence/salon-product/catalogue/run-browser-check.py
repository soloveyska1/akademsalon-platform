...
name=sys.argv[1]
s=Path('/tmp/salon-catalog-'+name+'.js').read_text().replace("u.endsWith('/sw.js')", "/\\/sw\\.js(?:\\?|$)/.test(u)")
...
Path('/tmp/salon-catalog-'+name+'-isolated.js').write_text(wrapped)
...
Path('/tmp/salon-catalog-'+name+'.log').write_text(r.stdout+r.stderr);print(r.stdout.split('### Ran Playwright code')[0])

async (page) => {
  ...
  await page.goto('http://127.0.0.1:8774/prolog.html');
    ...
    for (const width of [320,360,390,768,1024,1280,1440]) {
      ...
      for(let stage=0;stage<5;stage++) {
        ...
        if([390,1440].includes(width) && [0,1,3,4].includes(stage)) {
          ...
          await page.screenshot({path:'output/playwright/journey-'+theme+'-'+width+'-'+(stage+1)+'.png',fullPage:true});
        }
      }
    }
  }
  ...
  await page.goto('http://127.0.0.1:8774/prolog.html#prolog-step-3');

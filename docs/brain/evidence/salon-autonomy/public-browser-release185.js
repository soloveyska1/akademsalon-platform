async initial=>{
 ...
 page.on('pageerror',e=>errors.push(e.message));page.on('response',r=>{if(r.url().startsWith('http://127.0.0.1:8772/')&&r.status()>=400)requests.push({url:r.url(),status:r.status()})});
 await ctx.route('**/*',r=>{const u=r.request().url();if(u.includes('/api/'))return r.fulfill({json:u.endsWith('/features')?{ok:true,email_login:true,vk_login:true,pay_online:true,community_rewards:true}:{ok:true,authenticated:false,user:null,orders:[]}});if(u.startsWith('http://127.0.0.1:8772/'))return r.continue();return r.fulfill({status:200,body:'',contentType:'text/plain'})});
 ...
 for(const width of [360,390,768,1024,1280,1440])for(const theme of ['light','dark']){
  ...
  for(const route of routes){
   await page.goto('http://127.0.0.1:8772'+route,{waitUntil:'domcontentloaded'});await page.waitForTimeout(110);
   ...
   if((width===390||width===1440)&&theme==='light'&&['/','/dashboard.html','/configurator.html','/specifikaciya.html'].includes(route))await page.screenshot({path:'docs/brain/evidence/salon-autonomy/'+route.replaceAll('/','').replace('.html','')+'-'+width+'.png',fullPage:true,animations:'disabled'});
  }
 }

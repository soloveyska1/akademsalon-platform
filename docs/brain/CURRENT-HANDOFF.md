...
- Production `current` is `release223-reading-9876e933`, source `9876e9336b2c815c1c75e3b2baf6715cf83a3a56`. Pinned overlay on exact release222 changes97 paths and adds2assets (524→526files). Compatibility `dist` deliberately remains `release203-09ccfca4`. Never align it by assumption or rebuild live from older canonical root HTML.
- All eight approved conveniences are live: typo/alias search with highlighting and suggestions (global and library); focus reading; mobile contents sheet; copy14existing examples/templates across4guides; expanded tables; native/copy links to guide sections and exact PDF pages; gentle incoming-content appearance; original paper illustrations in empty search/saved states and404. Reader enhancements cover25guides; shared shell91pages. Existing appearance, return, intake and measurement packages remain intact.
- Search is local, bounded160characters/12tokens, escaped and deduplicated; exact results precede aliases/fuzzy. Partial/default suggestions are labelled honestly. No new persisted queries, personal text, arbitrary URLs, telemetry or storage keys. Guide sharing uses public route/heading IDs without queries. Existing heading text, IDs, legacy aliases and TOC labels remain compatible.
- PDF sharing becomes available only after the current epoch/page paint succeeds. Strict `#pdf=ID&page=N` permits only known public documents and valid integer pages; direct links override bookmarks in fit mode. Hash-driven Back closes the viewer without rewriting destination, Forward and document-to-document history work, ordinary card opens remain separate. Native share cancellation is silent; denied copying gives a selectable field. OS share/delivery itself was simulated, not claimed.
- Focus reading preserves a semantic block/fraction, and mobile contents/table/copy dialogs retain focus, page position and original table DOM. Explicit Tab loops and synchronous dismissal fix both browser-chrome escape and the queued-close/double-Escape scroll race. Independent reading drift is≤0.35px. Table clones remove duplicate IDs and inactive actions.
- Motion is a160ms incoming-main opacity .88→1; navigation/header remain native and immediately interactive. Reduced motion, calm and motionoff suppress it; Back/Forward skips it. Do not restore the earlier cross-document snapshot prototype: it caused reproducible immediate-menu and aborted-transition failures. Final return and route regressions pass. Direct-hash browser smooth scrolling can complete after load; tests wait for the target to be visible instead of assuming load means settled scroll.
- Verified: clean-source owner21/21,91public routes,10viewport/theme states (360/390/768/1024/1440); prior ease22/22, return18/18, intake9/9; Node652pass/9existing skips, Brain39, strict validation, deploy8tests. Two independent final hash-bound GO reviews: UI8viewport/theme cases and safety10probes, no outstanding P0/P1/P2. Native Safari, physical-device and screen-reader certification are not claimed.
- Real apply→rollback222→forward223 passed health and102exact static checks each forward; known expertise301 destination verified. Live Chrome390/1440 in both themes passed4/4,10actual static hashes per profile, new paths plus existing settings/FAQ/catalogue/inline helper; JSerrors0, real APIwrites0. Backend/DB/prices/order/auth/commerce/analytics/service-worker bytes untouched. Tested rollback is static `current`→release222 only; never restore DB.
- Evidence: `docs/brain/evidence/salon-reading-20260924/`; scripts: `scripts/salon-reading-20260924/`. Integrated `WS-d1515e1200b64c5e86cb89ebc7ea02b0`, frozen verified result `90616e5a`, terminal revision `720bdb4d`. Keep task branch `codex/salon-reading-v2-20260924` at terminal. The initial bootstrap-only branch was abandoned before implementation due an unnecessary mutable handoff read dependency; v2 pins immutable canonical/live input.
- Final conflict scan: hard0,67reviewed terminal-worktree warnings explicitly accepted by integration owner; no new conflicting path. Original user-dirty `.claude/launch.json` excluded and preserved. All owned test browser contexts closed; no dev server or watcher started.
- Existing daily monitor `automation-5` rechecked ACTIVE,10:00 daily, read-only/quiet for unchanged non-actionable state. Next exact step: inspect the next full-day consented open→input→attempt→confirmed-success coverage and aggregate lead/payment totals against the actual live release. Missing historical events are not zero traffic; these UI changes do not prove conversion uplift.

## Previous verified release222 — superseded by release223

Sequential integration-owner update; historical records below are not current release pointers.

- Production `current` is `release222-ease-fa5ea915`, source `fa5ea9156fbd1315cb48412367af5860b059aa6e`; UI implementation `b264e4f2bf56b343f60a6239a56d8c0134bf497c`. Exact release221 overlay changes 101 paths, adds three assets and preserves all files (521 → 524). Compatibility `dist` deliberately remains `release203-09ccfca4`. Never align it by assumption or rebuild production from older canonical root HTML.
- All eight approved public comforts are live: header search reuses the existing search; system/light/dark theme; remembered reader size and line spacing; mobile filter edge cues and selected-chip reveal; individual FAQ share links; compact Listik preference; PDF loading outline and successful-page reveal; short press/save/FAQ feedback. Settings are under **Меню → Вид сайта**; reader controls remain above the article. Shared enhancement covers 91 HTML consumers (one has an existing canonical redirect), 32 FAQ pages and 25 guides.
- Preferences are finite local enums only. System theme removes the legacy manual override; light/dark retain the existing key. Choice applies before layout, syncs across tabs including visible custom-select labels, and survives persisted-pageshow handling after denied writes. Reading confirmations expire after 4.2 seconds without clearing a later unrelated notice. No task, contact, search, chat or file text is added to persistence or telemetry. FAQ copy strips query parameters and offers a selectable fallback.
- Existing single Listik launcher keeps its handlers and form placement; compact mode only changes its presentation. Existing mobile visibility policies remain. PDF loading reuses epoch/paintId, real error/retry, success-only bookmarks and the close/reopen guard. Calm and OS reduced-motion disable enhanced feedback. Existing return/recent pages, catalogue state, file cards/undo and form hints/errors/counters remain intact.
- Deterministic/browser proof: 22/22 new comfort scenarios including ten 360/390/768/1024/1440 light/dark states; all 91 local route checks; return 18/18; intake 9/9; Node 652 pass / 9 existing skips; Brain 39/39; strict validation. Two independent hash-bound GO reviews; settings text contrast minimum 5.80 light / 8.24 dark; no new open P0/P1/P2 in scope. Native Safari, physical devices and screen-reader certification are not claimed.
- One intake test initially measured a 2 px visual hover translation as a layout change. Independent baseline/candidate evidence proves identical stable layout, one trusted click and one validation event; the unchanged suite rerun passes. Test helpers must distinguish transform from layout. Reading resume uses saved section plus fraction. Sticky-header tests use physical mouse; enhanced-select keyboard tests settle focus/scroll before opening.
- Clean release build matches every tested byte. Actual apply → rollback release221 → forward release222 passed health HTTP 200 and 106 readback checks on each forward pass. `/expertise.html` has a pre-existing Nginx 301 to `/`; verifier now checks exact status/location and homepage bytes without following arbitrary redirects. The initial readback attempt auto-rolled back correctly; no server redirect rule or UI fix was needed. Eight fail-closed deploy tests and an independent release-safety review pass.
- Live Chrome at 390/1440 in both themes passed 4/4 with all eight changed asset hashes verified per profile, real settings keyboard, search/FAQ/reading/catalogue/PDF and preserved inline helper. JS errors 0, real API writes 0. Backend, DB, prices, auth/order/commerce/analytics contracts and service-worker bytes are unchanged. Admin/dashboard receive refreshed shared-app cache URLs; the new preference bridge activates only on shell consumers.
- Evidence: `docs/brain/evidence/salon-ease-20260924/`; scripts: `scripts/salon-ease-20260924/`; integrated `WS-3c1b84f7d7314927b53b393b5cb7b526`; frozen verified result `3d6228f0`; terminal revision `b88df473fcd8bb8b4ce13008f447bf9a14231785`. Keep the task branch at that terminal revision. Tested rollback target: `release221-return-4ee1f306`, static `current` only; never restore DB for this release.
- Existing daily monitor `automation-5` was rechecked ACTIVE, daily 10:00 Europe/Moscow, readonly and quiet unless meaningful actionable change. It reads the actual release and this handoff. Next exact step: inspect the next complete day of consented open → input → attempt → confirmed-success coverage and aggregate leads/payments. Missing historical events do not mean zero traffic; UI changes do not establish growth.
- Original user-dirty `.claude/launch.json` remains outside commits. All browser contexts closed; no development server or watcher started. Fresh integration conflict check: hard 0, same 65 previously reviewed terminal-worktree warnings explicitly accepted by the integration owner; no new warning.

## Historical canonical and production records (superseded pointers)

### Previous verified release221 — superseded by release222

- Production `current` is `release221-return-4ee1f306`, from implementation `4ee1f306baa13f77b477f16dddf50a470583eaac`. Pinned overlay on release220 changes68 public HTML pages, two existing runtime assets and adds two assets (519→521files). Compatibility `dist` deliberately remains `release203-09ccfca4`. Never align it by assumption or rebuild live from older canonical root HTML.
- Public browsing now has a compact “Недавно смотрел” disclosure in the menu, explicit article continuation, PDF sample page/zoom bookmarks with an intentional first-page alternative, and catalogue facet/speed/card-position return. Catalogue browsing state remains separate from saved order composition. Existing reading favourites remain intact.
- Storage is local-only, finite and bounded: five known public pages, up to25 versioned article checkpoints and20 versioned PDF checkpoints, logical30-day expiry. Catalogue uses session/history state plus a one-use internal-navigation timestamp because the site intentionally sends no referrer. Queries, arbitrary URLs, search text, contacts, file contents and private routes are excluded. Clear invalidates old history and disarms pending article/PDF writes across tabs. Denied/corrupt/expired storage degrades to existing behavior.
- Direct article anchors and direct catalogue anchors win. Initial page loading does not overwrite a reading checkpoint. PDF writes only after a current successful render, fits width on the current viewport and retains bounded manual zoom. A reproduced queued-close/immediate-keyboard-reopen race is fixed; the final close still cleans up and returns focus. No new backend, analytics event, consent field or product price.
- Verified:18/18 hermetic Chrome cases plus mobile Back→Forward→Back; ten360/390/768/1024/1440 light/dark states; keyboard, focus,44px targets, reduced motion, no overflow. Node652pass/9skip, Brain39/39, strict validation and two independent GO reviews. New menu contrast light7.80:1/dark8.86:1. A catalogue false alarm was traced to Playwright locator auto-scroll before pointerdown; exact physical mouse tests pass and the unnecessary workaround was removed. No openP0/P1/P2 in the scoped release.
- Whole521-file frozen rebuild equals the tested tree. Actual apply→rollback220→forward221 passed health and exact public hashes. Live Chrome390/1440 light/dark4/4 verified all four comforts, original form hints, focus and exact assets; JS errors0/API writes0. No real order, payment, chat or analytics event was submitted. Backend/DB unchanged. Safari, physical mobile devices and conversion uplift were not measured.
- Evidence: `docs/brain/evidence/salon-return-20260924/`; scripts: `scripts/salon-return-20260924/`; integrated `WS-41497b6369b048a48672e20d8ac32ddb`; frozen verified result `e8f9c993`; terminal revision `f1a8afe97f22e0a9b2796510f1520851034c7285`. Keep its task branch at that terminal revision. Tested rollback target: `release220-polish-9f987a75`, static current pointer only; never restore DB for this release.
- Prior release220 contact hints, inline errors/counters and inline Listik; release219 file cards/drop/undo/date/autogrow; release218 consented measurement and course-plan route remain preserved. `configurator.html`, all order/commerce/auth/private/analytics runtimes and service-worker bytes are unchanged by release221.
- Existing daily monitor `automation-5` is ACTIVE, daily10:00 Europe/Moscow, readonly and quiet unless meaningful actionable change. It reads the actual live release and this handoff. Next exact step: inspect the next complete day of consented open→input→attempt→confirmed-success coverage and aggregate leads/payments. Missing historical events do not mean zero traffic; instrumentation and UX changes do not themselves prove growth.
- Original user-dirty `.claude/launch.json` remains outside commits. All test browser contexts closed; no dev server or watcher left running.



### Previous verified release220 — superseded by release221

- Production `current` is `release220-polish-9f987a75`, from verified implementation `9f987a75899b7c21203513f50d291eae20ecb552`. It overlays four files on release219, preserving every existing file and adding two assets (517 -> 519). Compatibility `dist` intentionally remains `release203-09ccfca4`. Never align it by assumption or rebuild live from older canonical root HTML.
- Direct intake now has optional Telegram/email/phone/VK hints that change inputmode/placeholder while preserving the value, native text type and original union validator. Errors are shown next to fields and cleared on correction; pointer-triggered blur rendering waits until the click is completed. First invalid native select gets visible focus. Topic/details show near-limit UTF16 counters. No new required step, payload field, consent term, private storage or analytics event.
- Existing single Listik launcher is inline during form entry to avoid covering guidance; its dialog stays in body. After server-confirmed success, that same launcher returns to body and remains available with correct close focus. No duplicate helper or new chat request.
- Release219 comforts remain: validated picker/drop file cards, local photo previews, persistent undo with original File/id, quick local dates with clear, auto-growing description; frozen retries prevent attachment/date mutation. Release218 consented measurement and explicit guide -> course-plan handoff also remain; contract2.5.0. Channel hints alone, select search and prefills do not count as first input.
- Verified: nine targeted + nine comfort + ten funnel hermetic browser suites; ten viewport/theme states (360/390/768/1024/1440), keyboard/44px/reduced motion/privacy. Real held-pointer sequence, cancel, counters after topic changes, helper before/after confirmed mock success, native first-error focus, unknown submission retry identity and upload behavior all passed. Node652 pass/9skip; Brain39 pass; strict validation; two independent final GO reviews. P0/P1/P2=0 found in bounded patch.
- Actual apply -> rollback release219 -> forward passed health and exact public hashes. Live GET-only Chrome390/1440 light/dark passed four scenarios, new and prior controls, helper open/close and guide plan context. No real order/payment/analytics/chat message sent; backend/DB unchanged. Safari, physical mobile keyboards and conversion uplift were not measured.
- Evidence: `docs/brain/evidence/salon-form-polish-20260924/`; integrated `WS-12e03bf925e9442ebb34b9177fd0b47d`, frozen result `dca8642c`, terminal revision `c43618f5`. Keep the task branch at that terminal revision. Tested rollback target `release219-comfort-08c202c1`, static current pointer only; never restore DB for this release.
- Existing daily monitor `automation-5` remains active at10:00 Europe/Moscow, read-only and quiet for unchanged/non-actionable state. It reads actual live release and this handoff. Next: inspect the next full-day consented sample and fresh open -> input -> attempt -> confirmed-success coverage, plus meaningful aggregate leads/payments. Historical missing events do not mean zero traffic; new measurement coverage does not prove audience growth.
- Original user-dirty `.claude/launch.json` remains outside commits. All test browser contexts closed; no dev server or watcher left running.

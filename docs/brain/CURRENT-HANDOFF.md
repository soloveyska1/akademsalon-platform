...
- Production `current` is `release219-comfort-08c202c1`, from verified implementation `08c202c1d5a2b3ee3ae0e04d14c364d4b51ce61c`. It overlays five files on `release218-funnel-5e860b3c`, preserving every existing file and adding two assets (515 -> 517). Compatibility `dist` intentionally remains `release203-09ccfca4`; never align it by assumption or rebuild live from older canonical root HTML.
- Direct intake has file cards with type/size and safe local photo previews, native picker plus drag/drop with one validator, persistent undo retaining the original File/id, quick local-calendar deadlines with clear action, and an auto-growing task description. Existing purple palette is preserved. Busy/frozen requests disable attachment/date changes. Files say ready before server upload confirmation.
- Verified: nine targeted and ten inherited isolated browser suites; 360/390/768/1024/1440 light/dark, reduced motion, keyboard focus and 44px controls; duplicate/size/type/count checks, preview cleanup, trusted desktop drop, midnight/leap date/quote, details caret visibility, uncertain submit and upload retry with stable identities. Node 652 passed/9 skipped, Brain39 passed, strict validation; two independent final GO reviews. P0=0/P1=0 in bounded scope.
- Actual production apply -> rollback to release218 -> forward passed health and exact public hashes. Live GET-only Chrome 390/1440 light/dark passed new controls and guide -> plan/course -> plan/master context. No real test order, payment or analytics event was sent; backend and DB unchanged. Safari and customer conversion uplift were not measured.
- Evidence: `docs/brain/evidence/salon-comfort-20260924/`; integrated workstream `WS-da0a31bda1434828b2b651bb2e975810`, frozen result `8799978d`, terminal revision `83c999a7`. Keep its terminal branch at that revision. Rollback is the static current-pointer switch to release218, with no database restore.
- Release218's repaired consented open/input/validation/failure coverage and explicit guide-to-course-plan handoff remain. Analytics contract is 2.5.0. Custom-select search, automatic prefills and synthetic events do not count as first input; quick date actions and real file drops do. Historical missing events must not be presented as zero traffic or new coverage as audience growth. No lead/revenue uplift is claimed.
- Existing daily monitor `automation-5` remains active at 10:00 Europe/Moscow, read-only and quiet on unchanged/non-actionable state. It reads the actual live release and this handoff before proposing features. Next: inspect the next full-day consented sample and event freshness, open -> input -> attempt -> confirmed success, plus meaningful aggregate lead/payment changes.
- Original user change `.claude/launch.json` remains outside all commits. Test browser contexts closed; no dev server or watcher left running.

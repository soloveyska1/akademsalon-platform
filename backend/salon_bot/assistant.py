...
def answer(question, order=None, context=None):
    ...
    if store_context and re.search(r'куп|покуп|материал|скид|бонус|промокод|семестр|скач|файл|лиценз|тираж|возврат|вернуть.*деньг',q):
        ...
        if page:
            ...
            r=result('store','У готовых материалов свои условия, отдельные от индивидуальных работ.\n\n'+body+'\n\nПеред оплатой проверь итог и доступные выгоды в корзине материалов.',[('Готовые материалы','/shop.html'),('Мои покупки','/shop.html#purchases')],['Как купить готовые материалы?','Какие бонусы в магазине?','Позови мастера'])
            r['sources']=[{'title':'Условия магазина материалов','url':'/shop-terms.html'}];return finish(r)

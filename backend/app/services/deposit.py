...
кошелька — аванс: тратятся только на оплату этапов заказов; данные для
чека НПД передаются при пополнении, а внутреннее списание аванса само
по себе не является новым денежным расчётом.

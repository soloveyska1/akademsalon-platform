...
(function (host) {
  ...
  function referrals(state,authenticated,legacy) {
    ...
    if(!state.enabled){
      var previous='';try{var oldLink=new URL(legacy&&legacy.ref_link_tg);if(oldLink.protocol==='https:'&&oldLink.hostname==='t.me'&&oldLink.pathname==='/academic_saloon_bot'&&/^ref_-?\d+$/.test(oldLink.searchParams.get('start')||''))previous='<div class="desk-circle-use"><h3>Действующая программа: 200 бонусов</h3><p>Однократно за первый полностью оплаченный заказ нового клиента. Бонусы действуют 90 дней.</p><label for="desk-circle-link">Твоя действующая личная ссылка</label><div class="desk-circle-link"><input id="desk-circle-link" readonly value="'+esc(oldLink.href)+'"><button class="desk-button" data-circle-copy>Копировать</button></div></div>';}catch(e){}
      return intro+'<p>Поделись личной ссылкой. После первого полностью оплаченного заказа друга получишь 200 бонусов на следующий заказ.</p><a class="desk-button" href="referral.html">Условия приглашений →</a><button class="desk-button" data-tab="wallet">Мои действующие бонусы</button>'+previous+'</section>';
    }

...
(function () {
  ...
  (function mobileEdition() {
    ...
    try {
      link.href = source ? new URL('../css/mobile.css?v=20260806shell123&glass=20260804glass1', source).href
        : 'assets/css/mobile.css?v=20260806shell123&glass=20260804glass1';
    } catch (e) {
      link.href = 'assets/css/mobile.css?v=20260806shell123&glass=20260804glass1';
    }
  ...
  window.SalonServices = [
    ...
    ,
    { id:'psychologyvip', label:'ВКР по психологии · полный проект', from:91000, unit:'', code:'pv', fixed:true,
      desc:'Полное сопровождение сложной доработки ВКР по психологии: исследовательский паспорт, статистический план и расчёты по обезличенным данным, поэтапная редактура, нормоконтроль, презентация, речь и подготовка к защите. 91 000 ₽ при плановом сроке и согласованном составе.',
      ask:[
        { id:'scope', label:'Что изменилось в работе?', short:'Исходная задача', type:'chips', req:true,
          opts:['Изменилась тема','Нужно перестроить главы','Нужна статистика','Есть замечания руководителя','Нужен полный цикл'] },
        { id:'data', label:'Что есть для эмпирической части?', short:'Данные', type:'chips', req:true,
          opts:['Обезличенная таблица','Только результаты методик','Данные ещё собираются','Нужна проверка комплекта'] }
      ] },
  ...
  var SEARCH = [
    ...
    ['diplomnaya-po-psihologii.html', 'Диплом по психологии', 'психология вкр клиническая психология корреляция корреляционный анализ манн уитни манна уитни сравнение групп статистика детектор ии полное сопровождение'],
  ...
  function createOrderContract(env) {
    ...
    function resolveState(producerId, spec, scope, body) {
      ...
      return Promise.resolve(calculated).then(function (fingerprint) {
        ...
        /* Тот же текст — тот же номер попытки: сервер узнает повтор и не заведёт
           второе дело. Это и есть работа идемпотентности. */
        if (loaded.state && loaded.state.intent_sha256 === fingerprint) {
          return { state: loaded.state, key: loaded.key };
        }
        /* Текст изменился, а прошлая попытка осталась неподтверждённой —
           РОТАЦИЯ: старый номер уходит в отставку, у новой правки свой.
           Раньше здесь стоял глухой отказ 'intent_changed', и человек читал
           «верните прежний вариант или напишите мастеру напрямую». Вернуть
           прежний вариант он обычно не мог — он его уже переписал, — а состояние
           лежит в sessionStorage и переживает перезагрузку. Выход был ровно один
           и неочевидный: закрыть вкладку. На практике человек просто уходил.
           Особенно больно это ударило 26.07–03.08, когда каждая отправка падала
           в 500 (см. историю с триггером акцепта): 500 — это ни подтверждение,
           ни окончательный отказ, поэтому состояние оставалось, и первая же
           правка формы запирала посетителя до закрытия вкладки.
           Чем платим за ротацию: если прошлая попытка всё-таки долетела до
           сервера, мастер увидит два обращения — прежнее и уточнённое. Дубль
           виден и правится в админке, потерянный клиент — нет. Денег на этом
           шаге не списывается (оплата идёт отдельным шагом), так что рисковать
           тут нечем. */
        var id = '';

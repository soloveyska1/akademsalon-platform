...
test('every redesigned route retains analytics exclusion before the shared runtime',()=>{const routes=fs.readdirSync(root).filter(p=>p.endsWith('.html')&&read(p).includes('salon-direct concept-shell'));for(const name of ['index.html','services.html','configurator.html'])assert.ok(routes.includes(name));for(const p of routes){const h=read(p);assert.ok(h.indexOf('configurator-nav-guard.js')<h.indexOf('assets/js/app.js'),p);assert.match(h,/analytics-v2.js\?v=20260829analytics4/);}});

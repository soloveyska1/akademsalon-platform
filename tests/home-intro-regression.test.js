...
test('intro is a home-only layer before the existing content landmark', () => {
  ...
  assert.match(home, /Одно дело исследования[\s\S]*— от темы до защиты\./);
  assert.doesNotMatch(
...
test('intro fails open and respects repeat, calm and reduced-motion preferences', () => {
  ...
  assert.match(home, /sessionStorage\.getItem\('salon_home_intro_v4'\)/);
  assert.match(home, /localStorage\.getItem\('salon_home_intro_v4'\)/);
  assert.match(home, /seenThisVersion/);
  assert.match(home, /var shouldPlay = replay && !skip/);
